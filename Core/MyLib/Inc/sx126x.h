...
#define OPCODE_READ_REGISTER            0x1d        // Чтение из регистра
...
#define OPCODE_CALIBRATE_IMAGE          0x98        // Каллибровка изображения
#define OPCODE_CALIBRATE                0x89        // Каллибровка чипа
#define OPCODE_PA_CONFIG                0x95        // Усилитель мощности
#define OPCODE_SET_BUFER_BASE_ADDRESS   0x8F        // Адресса Rx и Tx
#define OPCODE_GET_PACKET_TYPE          0x8F        // Получить тип пакета
/************ PacketType *****************/
...
#define SX126X_HEADER_EXPLICIT          0x00
#define SX126X_HEADER_IMPLICIT          0x01
...
void sx126x_SendData(SPI_HandleTypeDef* hspi, sx126x_cfg* modem,
...
void SetPaConfig(SPI_HandleTypeDef* hspi, uint8_t paDutyCycle, uint8_t hpMax,
...
void setBufferBaseAddress(SPI_HandleTypeDef* hspi, uint8_t txBaseAddress,
                                                    uint8_t rxBaseAddress);
void WriteReg(SPI_HandleTypeDef* hspi, uint16_t adress, uint8_t* data,
                                                        uint16_t len);
void SetPacketParams(SPI_HandleTypeDef* hspi, uint16_t preamble_len,
uint8_t header_type, uint8_t payload_len, uint8_t crc_type, uint8_t invert_iq);
...
void sendOpcode(SPI_HandleTypeDef* hspi, uint8_t opCode, uint8_t* pData,
                                                    uint16_t param_len);
void sendOpcodeAddressReadReg(SPI_HandleTypeDef* hspi, uint8_t opCode,
                uint8_t* pData, uint16_t param_len, uint8_t* address);
...
void SetDioIrqParams(SPI_HandleTypeDef* hspi, uint16_t irq_mask,
